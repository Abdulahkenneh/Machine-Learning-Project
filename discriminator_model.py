...
# DataLoader for pre-processed images
transform = transforms.Compose([
    transforms.ToTensor(),  # Converts PIL Image to Tensor
])

# Use the path to your processed images folder
dataset = datasets.ImageFolder(root='preprocessed_frames', transform=transform)
dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

# Training Loop
for epoch in range(epochs):
    for batch_idx, (real, _) in enumerate(dataloader):
        real = real.view(-1, img_dim).to(device)  # Flatten the image tensor
